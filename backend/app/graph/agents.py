...
def build_llm(
    ...
    """
    Construct a chat model from provided settings or environment variables.
    """
    provider = settings.provider if settings else "azure"
    ...
    if provider == "openai":
        ...
        model = settings.model if settings and settings.model else deployment or "gpt-4o"
        return ChatOpenAI(
            api_key=api_key,
            model=model,
        ...
        base_url = (
            settings.base_url if settings and settings.base_url else "http://localhost:1234/v1"
        )
        model = settings.model if settings and settings.model else "local-model"
        api_key = settings.api_key if settings and settings.api_key else "lm-studio"
        return ChatOpenAI(
            base_url=base_url,
            api_key=api_key,
            model=model,
        ...
        azure_deployment = (
            ...
            else deployment or os.environ.get("AZURE_OPENAI_DEPLOYMENT", "gpt-4o")
        )
...
def create_code_agent(
    ...
    tools = [
        _make_gated_tool(
            "run_python",
            "Execute Python code in a Docker sandbox (no network).",
            run_python,
            firewall,
            aid,
            "run_python",
        ...
        _make_gated_tool(
            "lint_code",
            "Lint Python code with ruff and return diagnostics.",
            _lint_code,
            firewall,
            aid,
            "lint_code",
        ...
        Tool(
            name="read_file",
            description="Read a file path from the sandbox output directory.",
...
def create_api_agent(
    ...
    tools = [
        Tool(
            name="call_api",
            description="Make an HTTP API call. Input: JSON {method, url, headers, body}.",
        ...
        _make_gated_tool(
            "validate_schema",
            "Validate JSON against a JSONSchema. Input: {schema, data}.",
            _validate_schema,
            firewall,
            aid,
            "validate_schema",
